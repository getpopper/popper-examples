...
do
  ...
  do
    ...
    do
      ...
      echo pgbench | docker run -i --rm --link pg$image:postgres postgres:$image pgbench -h postgres -U postgres -M $query_mode -t $n_threads > $GITHUB_WORKSPACE/pipelines/pgbench/results_/$image-$n_threads-$query_mode.results
    done
  done
